...
export default class BaseRenderer {
  ...
  updateClusters(camera, viewMatrix, scene) {
    ...
    for(let l = 0; l < NUM_LIGHTS; ++l)
    {
      ...
      //Get the height, width and length of the camera view space 
      var height = 2.0 * Math.tan((camera.fov / 2.0) * (Math.PI / 180.0)); 
      ...
      var z_min = Math.max(0, Math.min(Math.floor(lightPos[2] - lightRadius + (0.5 * depth) / slice_Z), this._zSlices - 1)); 
      //max
      ...
      var z_max = Math.max(0, Math.min(Math.floor(lightPos[2] + lightRadius + (0.5 * depth) / slice_Z), this._zSlices - 1)); 
